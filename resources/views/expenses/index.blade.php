        ...
        <div class="col">
            <nav aria-label="breadcrumb" class="rounded-3  m-2 ">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active"> قسم الصادرات والمصروفات اليومية</li>
                </ol>
            </nav>
        </div>
    </div>
            ...
            <div class="card">
                @if (isset($startDate) and isset($endDate))
                    <div class="card-header bg-info text-white">
                        <p>تقرير بالصادرات والمصروفات التشغيلية  من تاريخ {{ $startDate }} الي تاريخ {{ $endDate }}</p>
                    </div>
                @endif
                            ...
                            <tbody>
                                @if (isset($expenses))
                                    @foreach ($expenses as $expense)
                                        <tr>
                                            <td>
                                                <span class="badge bg-info text-dark">{{ $expense->type }}</span>
                                            </td>
                                            <td>{{ $expense->date }} </td>
